...
with tf.name_scope("Conv2"):
    taille_2 = 128
    with tf.name_scope("Group1"):
        filtres_conv21 = weight([5,5,taille_1,taille_2])
        b_conv21 = bias([taille_2],1.0)
    ...
    with tf.name_scope("Group2"):    
        filtres_conv22 = weight([5,5,taille_1,taille_2])
        b_conv22 = bias([taille_2],1.0)
    ...
    h_pool2 = tf.concat([h_pool21,h_conv22],axis=3, name="Concatenation")

with tf.name_scope("Conv3"):
    taille_3 = 192
    with tf.name_scope("Group1"):    
        filtres_conv31 = weight([3,3,taille_2*2,taille_3])
        b_conv31 = bias([taille_3],0.0)
        h_conv31 = tf.nn.relu(conv2d(h_pool2, filtres_conv31) + b_conv31)
    with tf.name_scope("Group2"):
        filtres_conv32 = weight([3,3,taille_2*2,taille_3])
        b_conv32 = bias([taille_3],0.0)
        h_conv32 = tf.nn.relu(conv2d(h_pool2, filtres_conv32) + b_conv32)
    
with tf.name_scope("Conv4"):
    taille_4 = 192
    with tf.name_scope("Group1"):
        filtres_conv41 = weight([3,3,taille_3,taille_4])
        b_conv41 = bias([taille_4],1.0)
        h_conv41 = tf.nn.relu(conv2d(h_conv31, filtres_conv41) + b_conv41)
    with tf.name_scope("Group2"):
        filtres_conv42 = weight([3,3,taille_3,taille_4])
        b_conv42 = bias([taille_4],1.0)
        h_conv42 = tf.nn.relu(conv2d(h_conv32, filtres_conv42) + b_conv42)
    
with tf.name_scope("Conv5"):
    taille_5 = 128
    with tf.name_scope("Group1"):
        filtres_conv51 = weight([3,3,taille_4,taille_5])
        b_conv51 = bias([taille_5],1.0)
    ...
    with tf.name_scope("Group2"):
        filtres_conv52 = weight([3,3,taille_4,taille_5])
        b_conv52 = bias([taille_5],1.0)
...
with tf.name_scope("Dense1"):
    taille_6 = 2048
    with tf.name_scope("Group1"):
        w_fc11 = weight([size_51,taille_6])
        b_fc11 = bias([taille_6],0.0)
        fc11 = tf.nn.relu(fully_connected_layer(h_pool51, w_fc11, b_fc11))
        fc1_drop1 = tf.nn.dropout(fc11, keep_prob)
    with tf.name_scope("Group2"):
        w_fc12 = weight([size_52,taille_6])
        b_fc12 = bias([taille_6],0.0)
