...
input = tf.placeholder(tf.float32, [None,28,28,3], name="input")
...
with tf.name_scope("Conv1"):
    with tf.name_scope("Group1"):
        filtres_conv11 = weight([11,11,3,48])
        b_conv11 = bias([48],0.0)
    ...
    with tf.name_scope("Group2"):
        filtres_conv12 = weight([11,11,3,48])
        b_conv12 = bias([48],0.0)
...
with tf.name_scope("Conv2"):
    with tf.name_scope("Group1"):
        filtres_conv21 = weight([5,5,48,128])
        b_conv21 = bias([128],1.0)
    ...
    with tf.name_scope("Group2"):    
        filtres_conv22 = weight([5,5,48,128])
        b_conv22 = bias([128],1.0)
...
with tf.name_scope("Conv3"):
    with tf.name_scope("Group1"):    
        filtres_conv31 = weight([3,3,256,192])
        b_conv31 = bias([192],0.0)
        h_conv31 = tf.nn.relu(conv2d(h_pool2, filtres_conv31) + b_conv31)
    with tf.name_scope("Group2"):
        filtres_conv32 = weight([3,3,256,192])
        b_conv32 = bias([192],0.0)
        h_conv32 = tf.nn.relu(conv2d(h_pool2, filtres_conv32) + b_conv32)
    
with tf.name_scope("Conv4"):
    with tf.name_scope("Group1"):
        filtres_conv41 = weight([3,3,192,192])
        b_conv41 = bias([192],1.0)
        h_conv41 = tf.nn.relu(conv2d(h_conv31, filtres_conv41) + b_conv41)
    with tf.name_scope("Group2"):
        filtres_conv42 = weight([3,3,192,192])
        b_conv42 = bias([192],1.0)
        h_conv42 = tf.nn.relu(conv2d(h_conv32, filtres_conv42) + b_conv42)
    
with tf.name_scope("Conv5"):
    with tf.name_scope("Group1"):
        filtres_conv51 = weight([3,3,192,128])
        b_conv51 = bias([128],1.0)
        ...
        h_pool51 = tf.reshape(h_pool51,[-1,3*3*128])
    with tf.name_scope("Group2"):
        filtres_conv52 = weight([3,3,192,128])
        b_conv52 = bias([128],1.0)
