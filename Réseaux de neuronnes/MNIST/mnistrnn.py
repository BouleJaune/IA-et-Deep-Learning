...
def rnn_model(x, learning_rate, n_h_layersn, n_per_layer, activation_f):
    
    layer = {'weights':tf.Variable(tf.truncated_normal([rnn_size, n_classes] ,stddev=0.1,seed=123), name='Weights'),
             'biases':tf.Variable(tf.constant( 0.0 , shape=[n_classes]), name='Biases')}
    ...
    output = tf.add(output, layer['biases'])
...
def neural_network_model(data,learning_rate, n_h_layers,
    ...
    for i in range(n_h_layers-1):
        with tf.name_scope('Hidden-layer-'+str(i+1)):
            layer_tmp = {'weights':tf.Variable(tf.random_normal([n_neurones[i], n_neurones[i+1]] ,stddev=0.1,seed=123), name='Weights'),
    ...
    with tf.name_scope('layer-'+str(n_h_layers)):                         
        output_layer = {'weights':tf.Variable(tf.random_normal( [n_neurones[n_h_layers],n_neurones[n_h_layers+1]] ,stddev=0.1,seed=123), name='Weights'),
...
def train_neural_network(learning_rate, n_h_layers,
    ...
    with tf.name_scope('Input-Data'):
        x_input = tf.placeholder(tf.float32,[None, n_chunks, chunk_size])
        
    log_dir = log_dir_name(learning_rate, n_h_layers, n_per_layer)    
            
    prediction = rnn_model(x_input,learning_rate, n_h_layers,
                            n_per_layer, activation_f)                                     
    ...
    with tf.Session() as sess:
        ...
        epoch=1
        accuracy_vec=[]
        feed_test = {x_input: mnist.test.images.reshape((-1,n_chunks,chunk_size)), y: mnist.test.labels}    
        while epoch <= n_epochs:                 
            ...
            for i in range(int(mnist.train.num_examples/batch_size)):
                ...
                x_b = x_b.reshape((batch_size,n_chunks,chunk_size))
